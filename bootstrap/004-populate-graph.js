...
const nodes = [
  ...
  { id: 'type-node-type', type: 'define-type', name: 'Node type enum', description: 'All node types: implementation subtypes of intent plus grammar inscription kinds.', test_condition: 'Enum type exists in database with all 27 values', test_verification: "SELECT enumlabel FROM pg_enum WHERE enumtypid = 'gdd.node_type'::regtype", build_instructions: "CREATE TYPE gdd.node_type AS ENUM ('define-table', 'define-type', 'define-schema', 'implement-operation', 'implement-endpoint', 'implement-traversal', 'implement-projection', 'implement-mutation', 'integrate', 'derive', 'translate', 'constrain-permission', 'constrain-invariant', 'establish-convention', 'define-vocabulary', 'compose', 'gap', 'decision', 'signal', 'expression', 'axiom', 'edge-node', 'actor', 'projection', 'retro-projection', 'commentary', 'test'). 27 values total. The first 21 are implementation-granularity subtypes of intent. 'edge-node' marks edge phenomena as first-class graph inscriptions. The next 4 are grammar inscription kinds. 'test' makes test conditions addressable as first-class nodes (targetable by infers-test, commentary, refinement) rather than only embedded fields." },
  ...
  { id: 'op-query-incomplete', type: 'implement-traversal', name: 'Query incomplete intents', description: 'Return red, current intents, gaps, and compose nodes within a board or graph scope.', test_condition: 'Requires board_id, graph_id, or explicit scope=global. Returns only red, current intents, gaps, and compose nodes within scope. Compose nodes are red when not all contains children are green. Excludes expression, decision, signal, axiom, test, actor, projection, retro-projection, commentary, edge-node nodes. Supports workable filter. Orders by downstream count.', test_verification: 'Integration test with mixed node states. Test rejection when no scope provided.', build_instructions: "Implement queryIncomplete(params) that returns red (no incoming satisfies edge), current (not superseded) intents and gaps. Accept: workable (bool), graph_id (optional), board_id (optional), scope (optional, only valid value is 'global'). SCOPE REQUIREMENT: at least one of board_id, graph_id, or scope='global' must be provided. If none provided, return an error: 'queryIncomplete requires board_id, graph_id, or scope=global. No one plays the whole graph.' Board_id filter: join with node_board_memberships or match nodes.board_id. Graph_id filter: join with graph_memberships. scope='global' is the explicit opt-in for whole-graph queries -- it works but should be treated as a diagnostic mode, not the normal operating mode. Exclude expression/decision/signal/axiom/test/actor/projection/retro-projection/commentary/edge-node nodes. Include gap nodes. Workable filter: only return nodes whose blocked-by dependencies are all green. Order by downstream dependent count desc. Compose nodes are green when all contains children are green." },
  ...
  // Phase 5b: Test nodes and signal relations
  { id: 'op-create-test', type: 'implement-operation', name: 'Create test node', description: 'Create a test node and link it to intents via tested-by edges. Makes tests addressable as first-class graph citizens rather than only embedded fields.', test_condition: 'Creates test node (type=test) with condition/verification/scope in artifacts JSONB. Creates tested-by edges to target intents. Optionally backfills intent embedded test_condition if currently null.', test_verification: 'Integration test: create test node linked to intent, verify tested-by edge and optional backfill.', build_instructions: "Implement createTest(params) that creates a test node and links it to intents. Accept: name, condition (text -- the test condition), verification (optional text), scope (optional text), intent_ids[] (required -- intents this test covers), id (optional), board_id (optional). Steps: (1) create node with type='test', condition/verification/scope stored in artifacts JSONB, condition also in notes, (2) for each intent_id, create tested-by edge from test node to intent, (3) optionally: if an intent's test_condition is currently null, copy condition into it for backward compatibility. Return { test_node, edges }." },
  { id: 'op-query-tests', type: 'implement-traversal', name: 'Query test nodes', description: 'Query test nodes, optionally filtered by intent or board.', test_condition: 'Returns test nodes with their tested-by targets. Filterable by intent_id and board_id.', test_verification: 'Integration test: create tests on multiple intents, query by intent and board.', build_instructions: "Implement queryTests(params). Accept: intent_id (optional -- return tests that have tested-by edges to this intent), board_id (optional). Query gdd.nodes WHERE type='test'. If intent_id provided, filter to tests with outgoing tested-by edge to that intent. If board_id provided, filter by board_id. For each test, include tested-by targets. Return array." },
  { id: 'op-create-signal-relation', type: 'implement-operation', name: 'Create signal relation', description: 'Create a signal node and signals edges to target nodes. The first inscription for an uncertain observation -- reception before interpretation.', test_condition: 'Creates signal node with signals edges to targets. Signal persists independently of any subsequent transduction.', test_verification: 'Integration test: create signal relation to intent, verify signals edge and signal node.', build_instructions: "Implement createSignalRelation(params). Accept: name, description, source (text -- origin of the observation), observed_content (text -- what was observed), target_ids[] (required -- nodes this signal affects), id (optional), board_id (optional). Steps: (1) create node with type='signal', source and observed_content in notes, source also in artifacts JSONB, (2) for each target_id, create signals edge from signal node to target, (3) return { signal_node, edges }. Signal is the first inscription -- uncertain, situated, pre-interpretive. It does not create intents or gaps; that is transduction's job." },
  { id: 'op-query-signals', type: 'implement-traversal', name: 'Query signal relations', description: 'Query signal nodes with their signals edges, optionally filtered by target, board, or source.', test_condition: 'Returns signal nodes with signals edge targets. Filterable by target_node_id, board_id, and source.', test_verification: 'Integration test: create signals to multiple targets, query by target and source.', build_instructions: "Implement querySignals(params). Accept: target_node_id (optional -- return signals with signals edge to this node), board_id (optional), source (optional text filter). Query gdd.nodes WHERE type='signal'. If target_node_id provided, filter to signals with outgoing signals edge to that node. If source provided, filter by artifacts JSONB containing that source. For each signal, include signals edge targets. Return array." },
...
const edges = [
  ...
  // Phase 5b: Test nodes and signal relations
  { from: 'op-create-test', to: 'op-create-intent', type: 'blocked-by' },
  { from: 'op-create-test', to: 'op-create-edge', type: 'blocked-by' },
  { from: 'op-query-tests', to: 'op-create-test', type: 'blocked-by' },
  { from: 'op-create-signal-relation', to: 'op-create-intent', type: 'blocked-by' },
  { from: 'op-create-signal-relation', to: 'op-create-edge', type: 'blocked-by' },
  { from: 'op-query-signals', to: 'op-create-signal-relation', type: 'blocked-by' },
...
async function populate() {
  ...
  try {
    ...
    // Show what's red (diagnostic -- excludes compose for simplicity; real queryIncomplete includes compose with derived green logic)
