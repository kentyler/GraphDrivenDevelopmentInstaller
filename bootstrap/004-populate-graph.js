...
const nodes = [
  ...
  { id: 'type-node-type', type: 'define-type', name: 'Node type enum', description: 'All node types: implementation subtypes of intent plus grammar inscription kinds.', test_condition: 'Enum type exists in database with all 26 values', test_verification: "SELECT enumlabel FROM pg_enum WHERE enumtypid = 'gdd.node_type'::regtype", build_instructions: "CREATE TYPE gdd.node_type AS ENUM ('define-table', 'define-type', 'define-schema', 'implement-operation', 'implement-endpoint', 'implement-traversal', 'implement-projection', 'implement-mutation', 'integrate', 'derive', 'translate', 'constrain-permission', 'constrain-invariant', 'establish-convention', 'define-vocabulary', 'compose', 'gap', 'decision', 'signal', 'expression', 'axiom', 'edge-node', 'actor', 'projection', 'retro-projection', 'commentary'). 26 values total. The first 21 are implementation-granularity subtypes of intent. 'edge-node' marks edge phenomena as first-class graph inscriptions (ordinary gdd.nodes rows, not a separate table). The last 4 are grammar inscription kinds added for grammar conformance." },
